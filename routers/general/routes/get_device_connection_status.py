...
async def get_iot_hub_connection_status(device_id):
    """
        Returns iotEdgeRuntime / iotHub connection states from IoT Hub twins.
    """
    responses = {}
    ...
    if responses[url1].status_code == 200:
        ...
        iot_edge_runtime = device_obj["connectionState"]    # Connected | Disconnected
    else:
    ...
    if responses[url2].status_code == 200:
        ...
        iot_hub = edge_hub_twin_obj.get("connectionState")     # Connected | Disconnected
    else:
    ...
    if iot_hub == "Connected" and iot_edge_runtime == "Disconnected":
        iot_edge_runtime = "Connected"

    return {"iotEdgeRuntime": iot_edge_runtime, "iotHub": iot_hub}


def get_sems_connection_status(smart_ems_device: dict) -> str:
    """
        Computes SEMS connectivity ("Connected"/"Disconnected") from an already-fetched
        SmartEMS.get_device_by_serial() result - no network call.
    """
    dt_last_seen = datetime.datetime.fromisoformat(smart_ems_device.get(
    ...
    return "Connected" if dt_delta < dt_max_delta else "Disconnected"
...
async def get_device_connection_status(device_id):
    ...
    resp.update(await get_iot_hub_connection_status(device_id))

    sems_obj = await SmartEMS.get_device_by_serial(device_id)
    resp["sems"] = get_sems_connection_status(sems_obj)
