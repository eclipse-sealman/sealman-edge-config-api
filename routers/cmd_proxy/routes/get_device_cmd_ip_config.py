...
async def get_device_cmd_ip_config(device: str, auth_context):
    module_name = "seal-app-cmd-proxy"
    module_method_name = "nm_show"
    method_payload = {
        "version": "1.0"
    }
    method_data = RequestDeviceModuleMethod(methodName="nm_show", methodPayload=method_payload)
    ...
    if resp.get("status") != 200:
        raise EdgeModuleAPIError(f"error while executing <{module_method_name}> on module <{module_name}>: "
                                 f"{resp.get('payload')}", status_code=resp.get("status"))
