...
async def get_device_cmd_status(device: str, 
                                _ = Depends(PathParamPermissionCheck(Device.READ_CMD_STATUS, Resource.DEVICE, "device"))):
...
async def get_device_ip_config(device: str, 
                                auth_context = Depends(PathParamPermissionCheck(Device.READ_IP_CONFIG, Resource.DEVICE, "device"))):
...
async def post_device_set_ip_static(device: str, ip_static: LanIpStatic,
                                    auth_context = Depends(PathParamPermissionCheck(Device.EDIT_IP_STATIC, Resource.DEVICE, "device"))):
...
async def get_device_fw_config(device: str, 
                               auth_context = Depends(PathParamPermissionCheck(Device.READ_CMD_FW_CONFIG, Resource.DEVICE, "device"))):
...
async def post_device_smartems_check(device: str, 
                                     auth_context = Depends(PathParamPermissionCheck(Device.EXECUTE_SMARTEMS_CHECK, Resource.DEVICE, "device"))):
...
async def get_device_config_show(device: str, 
                                 auth_context = Depends(PathParamPermissionCheck(Device.READ_CMD_CONFIG, Resource.DEVICE, "device"))):
