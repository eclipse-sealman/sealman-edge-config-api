...
ALLOW_STARTUP_WITHOUT_OIDC = os.getenv("ALLOW_STARTUP_WITHOUT_OIDC", "false").lower() == "true"
# use protocol http or https if provided, else use https
SEMS_URL = os.getenv("SEMS_URL")
if SEMS_URL:
    SEMS_URL = SEMS_URL if SEMS_URL.startswith(("http://", "https://")) else f"https://{SEMS_URL}"
SEMS_USER = os.getenv("SEMS_USER")
SEMS_PW = os.getenv("SEMS_PW")
SEMS_LOOKUP_INTERVAL = os.getenv("SEMS_LOOKUP_INTERVAL")
DEVICE_CACHE_INTERVAL = int(os.getenv("DEVICE_CACHE_INTERVAL", "60"))  # in seconds, default 1 minute

# Authentication Provider Configuration
_auth_provider = os.getenv("AUTHENTICATION_PROVIDER", "keycloak").lower()
if _auth_provider not in ["entra", "keycloak"]:
    raise ValueError(f"Invalid AUTHENTICATION_PROVIDER: '{_auth_provider}'. Valid values are 'Entra' or 'Keycloak'.")
...
BOOTSTRAP_SEMS_TEMPLATE_NAME = os.getenv("BOOTSTRAP_SEMS_TEMPLATE_NAME", "sealman-template")
...
BOOTSTRAP_IOTHUB_DEPLOYMENT_NAME = os.getenv("BOOTSTRAP_IOTHUB_DEPLOYMENT_NAME", "seal-base-deployment")
BOOTSTRAP_IOTHUB_TARGET_CONDITION = os.getenv("BOOTSTRAP_IOTHUB_TARGET_CONDITION", "tags.deployment='base'")
